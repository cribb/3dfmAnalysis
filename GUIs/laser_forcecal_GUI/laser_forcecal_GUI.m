function varargout = laser_forcecal_GUI(varargin)
% LASER_FORCECAL_GUI M-file for laser_forcecal_GUI.fig
%      LASER_FORCECAL_GUI, by itself, creates a new LASER_FORCECAL_GUI or raises the existing
%      singleton*.
%
%      H = LASER_FORCECAL_GUI returns the handle to a new LASER_FORCECAL_GUI or the handle to
%      the existing singleton*.
%
%      LASER_FORCECAL_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in LASER_FORCECAL_GUI.M with the given input arguments.
%
%      LASER_FORCECAL_GUI('Property','Value',...) creates a new LASER_FORCECAL_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before laser_forcecal_GUI_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to laser_forcecal_GUI_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @laser_forcecal_GUI_OpeningFcn, ...
                   'gui_OutputFcn',  @laser_forcecal_GUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before laser_forcecal_GUI is made visible.
function laser_forcecal_GUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to laser_forcecal_GUI (see VARARGIN)

% Choose default command line output for laser_forcecal_GUI
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes laser_forcecal_GUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = laser_forcecal_GUI_OutputFcn(hObject, eventdata, handles)
...
function pushbutton_compute_Callback(hObject, eventdata, handles)
    ...
    force_table = laser_forcecal(trackfile, magfile, dominant_pole, bead_radius * 1e-6, viscosity);

    assignin('base', 'force_table', force_table);
