...
% Last Modified by GUIDE v2.5 15-May-2006 16:50:12
...
if isempty(igood)
    ...
    [selec,ok] = listdlg('ListString',get(handles.lta.menu_files,'String'),...
        'InitialValue',1,...
        'OKstring','Select',...
        'Name','Select bead-on-cell freq sweep file(s)');
    if ok
        set(handles.button_selectFS,'UserData',igood(selec));
    end
end
...
if isempty(igood)
    ...
    if ok
        set(handles.button_selectFixed,'UserData',igood(selec));
    else
...
global g
dbstop if error
fsid = get(handles.button_selectFS,'UserData');
% pdid = get(handles.button_selectPD,'UserData');
fxid = get(handles.button_selectFixed,'UserData');
...
for c = 1:length(fids)
    ...
    if ~isequal(length(idx),length(magdata.info.param.fvec))
        ...
        if isequal(goon,'Debug');
            ...
            for dim = 1:4
                res(c).(rxyz(dim)).power = [];
                res(c).(rxyz(dim)).ftest = [];
            end
    ...
    end % testing if crossing detections matched    
