function varargout = varforce_drive_gui_beta(varargin)
% VARFORCE_DRIVE_GUI_BETA loads the GUI that drives the variable force calibration experimental protocol for 3dfm
%
% 3DFM function  
% Magnetics/varforce  
...
% VARFORCE_DRIVE_GUI_BETA M-file for varforce_drive_gui_beta.fig
%      VARFORCE_DRIVE_GUI_BETA, by itself, creates a new VARFORCE_DRIVE_GUI_BETA or raises the existing
%      singleton*.
%
%      H = VARFORCE_DRIVE_GUI_BETA returns the handle to a new VARFORCE_DRIVE_GUI_BETA or the handle to
%      the existing singleton*.
%
%      VARFORCE_DRIVE_GUI_BETA('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VARFORCE_DRIVE_GUI_BETA.M with the given input arguments.
%
%      VARFORCE_DRIVE_GUI_BETA('Property','Value',...) creates a new VARFORCE_DRIVE_GUI_BETA or raises the
%      existing singleton*.  Starting from the left, property value pairs
%      are
%      applied to the GUI before varforce_drive_gui_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to varforce_drive_gui_beta_OpeningFcn via varargin.
%
...
% Edit the above lbl_calib_visc to modify the response to help varforce_drive_gui_beta

% Last Modified by GUIDE v2.5 02-Aug-2012 12:06:24

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @varforce_drive_gui_beta_OpeningFcn, ...
                   'gui_OutputFcn',  @varforce_drive_gui_beta_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before varforce_drive_gui_beta is made visible.
function varforce_drive_gui_beta_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to varforce_drive_gui_beta (see VARARGIN)

    % Choose default command line output for varforce_drive_gui_beta
    handles.output = hObject;

    % Update handles structure
    guidata(hObject, handles);

    % UIWAIT makes varforce_drive_gui_beta wait for user response (see UIRESUME)
    % uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = varforce_drive_gui_beta_OutputFcn(hObject, eventdata, handles) 
...
% --- Executes on button press in factor_test_button.
function factor_test_button_Callback(hObject, eventdata, handles)
% hObject    handle to factor_test_button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    DAQid = get(handles.popupmenu_daqboard, 'String');
    DAQid_idx = get(handles.popupmenu_daqboard, 'Value'); 
    ...
    scaleFactor = str2num(get(handles.edit_factor, 'String'))
    
    params.voltages             = scaleFactor(1, 1)*str2num(get(handles.edit_voltages, 'String'));
    params.pulse_widths         = str2num(get(handles.edit_pulsewidths, 'String'));
    if size(scaleFactor, 2)>1
        params.pulse_widths = params.pulse_widths*scaleFactor(1, 2);
    end
    params.duration             = str2num(get(handles.lbl_duration, 'String'));
    params.calibrator_viscosity = str2num(get(handles.edit_calibrator_viscosity, 'String'));
    params.bead_radius          = str2num(get(handles.edit_bead_radius, 'String'));  
