...
% duplicate_cutoff = MSD cutoff value for removing duplicate particles; if
% 0, then no duplicate removal
...
j = 1; % Counter for # frames of each particle
...
for i = 2:length(A(:,1));
    ...
    if i == length(A(:,1)) % Last particle
        ...
        frames = [frames length(A((sum(frames)+1):end,1))];
        k = k+1;
    end
end
num_particles = k;
...
if min(frames) < min_frames
    for i = 1:num_particles
        ...
        if frames(k) < min_frames
           bad_frames = [bad_frames frames(k)];
           rows = sum(frames(1:k));
...
% (MAY ADD CUTOFF VS. PARTICLE DENSITY, ET.C)
if duplicate_cutoff > 0 && num_particles ~= 0
    ...
    if isempty(log_duplicates) == 0
        ...
        % Remove duplicates
        indices = find(unique(log_duplicates(:,2)));
...
if num_particles == 0
    ...
    summary_header = {'Particles',num_particles,[],[];'Avg Frames',mean(frames),[],[];[],[],[],[];'Bad Particles',length(bad_frames),[],[];'Avg Frames',mean(bad_frames),[],[];[],[],[],[];'Transport Modes:',[],[],[];[],[],'%','Avg Frames';'Active:',[],[],[];'Diffusive:',[],[],[];'Hindered:',[],[],[];'Immobile:',[],[],[];[],[],[],[];'Stuck particles (logMSD<-2)',num_stuck,[],[];'Avg Frames',avg_frames_stuck,[],[];[],[],[],[];'Moving particles (logMSD>=-2)',num_moving,[],[];'Avg Frames',avg_frames_moving,[],[]};
    ...
    % Classification calculates relative change (RC) values and classifies transport mode for each particle
    ...
    indices_stuck = find(log10(Deff)<-2);
    if isempty(indices_stuck) == 1
        ...
        avg_frames_stuck = mean(frames(indices_stuck));
    ...
    indices_moving = find(log10(Deff)>=-2);
    if isempty(indices_moving) == 1
        ...
        avg_frames_moving = mean(frames(indices_moving));
        for i = 1:length(indices_moving)
            XY_moving = [XY_moving; A(find(A(:,1)==indices_moving(i)),:)];
        end
    ...
    for i = 1:length(Timescale)
        geom_MSD(i) = geomean(nonzeros(MSD(i,:)));
        geom_MSD_stuck(i) = geomean(nonzeros(MSD_stuck(i,:)));
        geom_MSD_moving(i) = geomean(nonzeros(MSD_moving(i,:)));
    end
    
    geom_Deff = zeros(1,length(Timescale));
    for i = 1:length(Timescale)
        geom_Deff(i) = geomean(nonzeros(Def(i,:)));
    end
    ...
    summary_header = {'Particles',num_particles,[],[];'Avg Frames',mean(frames),[],[];[],[],[],[];'Bad Particles',length(bad_frames),[],[];'Avg Frames',mean(bad_frames),[],[];[],[],[],[];'Transport Modes:',[],[],[];[],[],'%','Avg Frames';'Active:',modes(1),modes(1)/num_particles*100,avg_frames(1);'Diffusive:',modes(2),(modes(2)/num_particles)*100,avg_frames(2);'Hindered:',modes(3),modes(3)/num_particles*100,avg_frames(3);'Immobile:',modes(4),(modes(4)/num_particles)*100,avg_frames(4);[],[],[],[];'Stuck particles (logMSD<-2)',num_stuck,[],[];'Avg Frames',avg_frames_stuck,[],[];[],[],[],[];'Moving particles (logMSD>=-2)',num_moving,[],[];'Avg Frames',avg_frames_moving,[],[]};
    xlswrite(filename,summary_header,'Summary','A1')
    ...
    [frames_edges,frames_binned,frames_percentages] = dist(frames,5);
    ...
    parameters_header = {'MPT PARAMETERS';[];'Conversion (um/pixel)';'Frame Rate (frames/s)';'Minimum # Frames';'Duplicate Cutoff (MSD; pixels^2)'};
...
for j = 1:num_particles
    ...
    for k = 1:frames(j)-1;
       MSD(k,j) = conversion^2*((sum((temp((k+1:end),3)-temp((1:(end-k)),3)).^2)+sum((temp((k+1:end),4)-temp((1:(end-k)),4)).^2))./(frames(j)-k));
       Def(k,j) = MSD(k,j)/4./Timescale(k)'; % MSD = 4*Def*Timescale for 2D particle tracking
    end
end


function [modes,avg_frames] = classification(num_particles,frames,MSD,Def)
...
for j = 1:num_particles
    ...
    if classify(j) == 3;
        a = a+1;
        act_frames = cat(1,act_frames,frames(j));
    elseif classify(j) == 2;
        b = b+1;
        diff_frames = cat(1,diff_frames,frames(j));
    elseif classify(j) == 1;
        c = c+1;
        hind_frames = cat(1,hind_frames,frames(j));
    else d = d+1;
        immob_frames = cat(1,immob_frames,frames(j));
    end
end
