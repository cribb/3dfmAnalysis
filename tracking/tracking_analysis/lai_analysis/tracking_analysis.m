...
for fr = 1:numframes+1
    matched_traj_fr = 0;
    extraA_fr = 0;
    extraB_fr = 0;
    for n = 1:length(data)
        if data(n,1) == fr-1 && ~isnan(data(n,2)) && ~isnan(data(n,3))
            matched_traj_fr = matched_traj_fr + 1;
        elseif data(n,1) == fr-1 && isnan(data(n,3)) %if B has no matching location, there is an extra one in A
            extraA_fr = extraA_fr + 1;
        elseif data(n,1) == fr-1 && isnan(data(n,2)) %if A has no matching location, there is an extra one in B
