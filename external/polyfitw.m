function [p,S,mu] = polyfitw(x,y,n,nul,w)
%POLYFITW Fit polynomial to data, with new features.
%
% The new usage:
%   POLYFITW(X,Y,N,NUL,W) finds the coefficients of a polynomial P(X) of
...
%
%  POLYFITW(X,Y,N,NUL) assumes that all weights of data are equal to 1.
%  POLYFITW(X,Y,N,[],W) allows to weight data without forcing any coefficient to be zero.
%
% The original usage is still available:
%   POLYFITW(X,Y,N) finds the coefficients of a polynomial P(X) of
%   degree N that fits the data, P(X(I))~=Y(I), in a least-squares sense.
%
%   The structure S contains the Cholesky factor of the Vandermonde
%   matrix (R), the degrees of freedom (df), and the norm of the
%   residuals (normr) as fields. Note that df also deppends on NUL vector.
%
