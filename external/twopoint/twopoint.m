...
% tracks: data to be used (in the form of posstot)
% params: [rmin, rmax, nbrbins, maxtime]
    % rmin: minimum separation distance (in micrometers) to compute the
        % correlation for
    % max: maximum separation distance (in micrometers) to compute the
        % correlation for
    % nbrbins: number log-spaced bins between those two boundaries
    % maxtime: maximum time (in frames) over which to compute correlation
% timestep: time interval between frames, in seconds
% dim: dimensionality of the data (2 or 3)
% mydts: optional vector of dt's for which correlations should be computed
%   (set to zero for computer created logspaced vector)
% dedrift: already done, always set to 0
...
%     filebased = 0
%     nf = 1
    % Finds maximum and minimum frame numbers in tracks
      erwmax=max(tracks(:,length(tracks(1,:))-1));
      erwmin=min(tracks(:,length(tracks(1,:))-1));
      % if the maxtime is larger than the time span included in the data,
      % it is changed to the time span of the data
      params(4) = min( params(4) , (erwmax-erwmin)*timestep );
% endelse
% 

% creating dt vector for use in computing correlatoins
if mydts ==0
    % generate the time partition-- about 10 points per decade
    % dt entirely whole numbers
    dt = round(1.15.^(0:99));
    [eldt,idt]=unique(dt);
    dt = dt(idt);
...
% for loop across each time step
for j=1:ndt
%        if filebased then $
...
%    disp(['dt equals: ' num2str(dt(j)) ' j=' num2str(j)])
    res = get_corr(trj,[lmin,lmax,lrbinsize,nbins],dt(j),dim);
