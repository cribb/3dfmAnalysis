function hh = logerrorbar(x, y, l,u,symbol)
%LOGERRORBAR Error bar plot for log-log plots.
%   LOGERRORBAR(X,Y,L,U) plots the graph of vector X vs. vector Y with
...
%
%   LOGERRORBAR(X,Y,E) or LOGERRORBAR(Y,E) plots Y with error bars [Y-E Y+E].
%   LOGERRORBAR(...,'LineSpec') uses the color and linestyle specified by
%   the string 'LineSpec'.  See PLOT for possibilities.
%
%   H = LOGERRORBAR(...) returns a vector of line handles.
%
%   For example,
%      x = 1:10;
%      y = sin(x);
...
%   $Revision: 1.1 $  $Date: 2006/08/01 02:30:19 $
...
symbol = [mark ls col]; % Use marker only on data part
esymbol = ['-' col]; % Make sure bars are solid
h = plot(x,y,symbol); hold on
h = [h; plot(xb,yb,esymbol)]; hold off
