...
window = [1 2 5 10 20 50 100 200 500 1000 2000 5000];
...
for m = 1:N_samples,
    ...
    for k = 1:size_window,
        ...
        for j = 1:(N_frames - window(k)), % start frames loop
            ...
            r2(j) = (M_data(m+1,j+window(k))*convert - M_data(m+1,j)*convert)^2 + (M_data(2*m+1,j+window(k))*convert - M_data(2*m+1,j)*convert)^2 + (M_data(3*m+1,j+window(k))*convert - M_data(3*m+1,j)*convert)^2;
