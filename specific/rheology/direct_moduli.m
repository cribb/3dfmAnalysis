function v = direct_moduli(t, J, J0, eta, dpoints, report)
...
for ww = 1:dpoints
    w = frange(ww);
    GStar = 1i*w./(1i*w*J0 + (1-exp(-1i*w*t(1)))*(J(1)-J0)/t(1) + exp(-1i*w*t(size(t,1)))/eta + sum(diff(J)./diff(t).*(exp(-i*w*(t(1:(size(t)-1))))-exp(-i*w*(t(2:size(t)))))));
