function v = laser_relaxtime(tracking_log_file, magnet_log_file, dominant_pole)
...
for k = 1 : ( length(magtime) - 1 );
    ...
    if (N > thresh) & (m.analogs(k, dominant_pole) == 0)
        ...
        [J, tau, R_square] = relaxation_time(beadtime(idx) - mintime, r(idx), 1);
        ...
        tau_table(count, 5) = tau;
        tau_table(count, 6) = R_square;
