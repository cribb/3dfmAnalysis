function hout = streamparticlesARS(varargin)
%
%   Adam edited this to save each drawn frame to current directory based on
%   saveOption (optional, default is 'n').  The filename must be changed 
%   manually, line 222 or so.
%
...
%
%   ARS modified this to allow saving each image frame. To enable saving
%   .tif images to the current directory, activate the block of code around
%   line ???
%
...
%   $Revision: 1.1 $  $Date: 2011/09/14 16:47:02 $



[cax,args,nargs] = axescheck(varargin{:});
[h, verts, n, animate, framerate, partalign, props, saveOption] = ...
...
set(h, 'linestyle', 'none', 'erasemode', 'xor', 'marker', 'o', ...
    'markeredgecolor', 'none', 'markerfacecolor', [0 .6 0], 'MarkerSize', 8);
...
%             % ars TRIED to modify this to plot streamlines, quivers, etc. 
%             % over stream particles and FAILED
%             if i == 1
%                 children = get(cax, 'Children')
%                 child1 = children(1);
%                 children(1) = [];
%                 children = [children; child1];
% 
%                 set(cax, 'Children', flipud(children));
% 
%                 childrn = get(cax, 'Children')
%             end
            ...
            for k = 1:inc;
                ...
                %%%%% HERE IS WHERE WE SAVE IMAGES TO MAKE A MOVIE %%%%%
                
                if strcmp(saveOption, 'y')
                    filename = strcat(['Couette' num2str(k+10*(i-1))]);
                    SaveFigure(filename, [], {'png'}, 75, [], [], [], 'n', 'n')
                                            % 'n' here chooses not to
                                            % save pdf or .fig
                end
                %%%%%%%%%%  ARS 12.14.2009 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [h, verts, n, animate, framerate, partalign, props, saveOption] = ...
...
if nin==0
    ...
    if nin>=2   % param value pairs
        ...
        while pos<nin
            ...
            if ~ischar(pname)
                error(id('NonStringPVPair'),'First element of property value pairs must be a string');
            end
            if pos+1>nin
                error(id('MissingPVPair'),'Missing value in property value pair.');
            end
            pval = vargin{pos+1};
            ...
            elseif strcmp(pname, 'saveoption')
                saveOption = lower(pval);
            else
                props.(pname) = pval;
            end
            pos = pos+2;
...
function str = id(str)
str = ['MATLAB:streamparticles:' str];
